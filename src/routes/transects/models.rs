use crate::routes::plots::models::Plot;
use crate::routes::transects::db;
...
impl CRUDResource for Transect {
    ...
    async fn create(
        ...
        for (i, node) in create_model.nodes.into_iter().enumerate() {
            ...
            let transect_node_active = crate::routes::transects::nodes::db::ActiveModel {
                id: Set(Uuid::new_v4()),
                plot_id: Set(plot.id),
                transect_id: Set(transect_id),
                order: Set(i32::try_from(i).unwrap()),
            };
            crate::routes::transects::nodes::db::Entity::insert(transect_node_active)
                .exec(db)
                .await?;
        }
    ...
    async fn get_all(
        ...
        let results: Vec<(db::Model, Vec<crate::routes::transects::nodes::db::Model>)> =
            Self::EntityType::find()
                ...
                .offset(offset)
                .limit(limit)
                .find_with_related(crate::routes::transects::nodes::db::Entity)
                .all(db)
                .await?;
        ...
        for (transect_model, nodes) in results {
            ...
            let area: crate::routes::areas::models::Area = crate::routes::areas::db::Entity::find()
                .filter(crate::routes::areas::db::Column::Id.eq(transect_model.area_id))
                .one(db)
                .await?
    ...
    async fn update(
        ...
        crate::routes::transects::nodes::db::Entity::delete_many()
            .filter(crate::routes::transects::nodes::db::Column::TransectId.eq(updated.id))
            .exec(db)
            .await?;
