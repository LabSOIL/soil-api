...
pub struct Sensor {
    ...
    pub area: Option<crate::routes::areas::models::Area>,
    #[crudcrate(update_model = false, create_model = false)]
    #[schema(no_recursion)]
    pub assignments:
...
impl CRUDResource for Sensor {
    ...
    async fn get_all(
        ...
        > = models
            .load_many(crate::routes::sensors::profile::assignment::db::Entity, db)
            .await?
            .pop()
            .unwrap()
            .into_iter()
    ...
    async fn get_one(db: &DatabaseConnection, id: Uuid) -> Result<Self::ApiModel, DbErr> {
        let (sensor, data) = Self::EntityType::find()
            ...
            .order_by_asc(crate::routes::sensors::data::db::Column::TimeUtc)
            .all(db)
            .await?
            .pop()
        ...
        let assignments: Vec<
            crate::routes::sensors::profile::assignment::models::SensorProfileAssignment,
        > = crate::routes::sensors::profile::assignment::db::Entity::find()
            .filter(crate::routes::sensors::profile::assignment::db::Column::SensorId.eq(id))
            .all(db)
            .await?
            .into_iter()
            .map(std::convert::Into::into)
    ...
    async fn create(
        ...
        if let Some(ref data_base64) = create_model.data_base64 {
            ...
            let new_data_result = crate::routes::sensors::services::process_sensor_data_base64(
                data_base64,
                result.last_insert_id,
            )
            .map_err(DbErr::Custom)?;

            let mut active_models: Vec<crate::routes::sensors::data::db::ActiveModel> = vec![];

            for obj in new_data_result {
                let active_model = crate::routes::sensors::data::db::ActiveModel {
            ...
            if !active_models.is_empty() {
                ...
                for chunk in active_models.chunks(CHUNK_SIZE) {
                    crate::routes::sensors::data::db::Entity::insert_many(chunk.to_vec())
                        .exec(db)
                        .await?;
                }
            }
    ...
    async fn update(
        ...
        if let Some(ref data_base64) = update_model.data_base64 {
            // Process the base64 string into SensorData objects
            let new_data_result =
                crate::routes::sensors::services::process_sensor_data_base64(data_base64, id)
                    .map_err(DbErr::Custom)?;

            // Query existing sensor data records for this sensor, sorted by time_utc descending
            let existing_data = crate::routes::sensors::data::db::Entity::find()
                .filter(crate::routes::sensors::data::db::Column::SensorId.eq(id))
                .order_by_desc(crate::routes::sensors::data::db::Column::TimeUtc)
                .all(db)
                .await;
    ...
    async fn delete_many(db: &DatabaseConnection, ids: Vec<Uuid>) -> Result<Vec<Uuid>, DbErr> {
        for id in &ids {
            let sensor_profile_assignments =
                crate::routes::sensors::profile::assignment::db::Entity::find()
                    .filter(
                        crate::routes::sensors::profile::assignment::db::Column::SensorId.eq(*id),
                    )
                    .all(db)
                    .await?;
...
impl Sensor {
    pub async fn get_one_low_resolution(
        ...
        for row in rows {
            ...
            let sensor_data = crate::routes::sensors::data::models::SensorData {
                instrument_seq: 0,
                time_utc: bucket,
        ...
        for window in aggregated_data.windows(2) {
            ...
            if window[1].time_utc - window[0].time_utc > gap_threshold {
                ...
                processed_data.push(crate::routes::sensors::data::models::SensorData {
                    instrument_seq: 0,
...
async fn get_data_range(
    ...
    let first_data = crate::routes::sensors::data::db::Entity::find()
        .filter(crate::routes::sensors::data::db::Column::SensorId.eq(sensor_id))
        .order_by_asc(crate::routes::sensors::data::db::Column::TimeUtc)
        .one(db)
        .await?;

    let last_data = crate::routes::sensors::data::db::Entity::find()
        .filter(crate::routes::sensors::data::db::Column::SensorId.eq(sensor_id))
        .order_by_desc(crate::routes::sensors::data::db::Column::TimeUtc)
        .one(db)
        .await?;
