...
impl SensorProfile {
    ...
    pub async fn get_one_high_resolution(
        ...
        let mut sensor_profile = Self::get_one(db, id).await?;
        for assignment in &mut sensor_profile.assignments {
            let sensor_data_records = crate::routes::sensors::data::db::Entity::find()
                .filter(crate::routes::sensors::data::db::Column::SensorId.eq(assignment.sensor_id))
                .filter(
                    crate::routes::sensors::data::db::Column::TimeUtc
            ...
            for record in sensor_data_records {
                let sensor_data: crate::routes::sensors::data::models::SensorData = record.into();
                data_vec.push(sensor_data);
            }
            assignment.data = data_vec;
        }
