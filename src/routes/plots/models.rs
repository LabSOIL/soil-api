...
pub struct Plot {
    ...
    #[crudcrate(update_model = false, create_model = false)]
    #[schema(no_recursion)]
    pub samples: Vec<crate::routes::samples::models::PlotSample>,
    #[crudcrate(update_model = false, create_model = false)]
    pub nearest_sensor_profiles: Vec<ClosestSensorProfile>,
    #[crudcrate(update_model = false, create_model = false)]
    pub transects: Vec<crate::routes::transects::models::Transect>,
}
...
impl
    From<(
        ...
        Vec<crate::routes::samples::db::Model>,
    ...
    )> for Plot
{
    fn from(
        ...
        let area: crate::routes::areas::models::Area = area_db.into();
        let samples: Vec<crate::routes::samples::models::PlotSample> = samples_db
            .into_iter()
            .map(crate::routes::samples::models::PlotSample::from)
            .collect();
...
impl CRUDResource for Plot {
    ...
    async fn get_all(
        ...
        let objs = Self::EntityType::find()
            ...
            .offset(offset)
            .limit(limit)
            // .find_also_related(crate::routes::areas::db::Entity)
            .all(db)
            .await
            .unwrap();
        ...
        for obj in objs {
            let area = obj
                .find_related(crate::routes::areas::db::Entity)
                .one(db)
                .await
                .unwrap()
                .unwrap();
    ...
    async fn get_one(db: &DatabaseConnection, id: Uuid) -> Result<Self, DbErr> {
        ...
        let transect_nodes = crate::routes::transects::nodes::db::Entity::find()
            .filter(crate::routes::transects::nodes::db::Column::PlotId.eq(id))
            // .find_also_related(crate::routes::transects::db::Entity)
            .all(db)
            .await;
        ...
        if let Ok(transect_nodes) = transect_nodes {
            for node in transect_nodes {
                let transect = node
                    ...
                    .unwrap()
                    .unwrap();
                transects.push(crate::routes::transects::models::Transect::from(transect));
            }
        }
