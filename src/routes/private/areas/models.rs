...
impl CRUDResource for Area {
    ...
    async fn get_all(
        ...
        for model in models {
            ...
            let mut plots = model
                .find_related(crate::routes::private::plots::db::Entity)
                .all(db)
                .await?;
            ...
            let soil_profiles = model
                .find_related(crate::routes::private::soil::profiles::db::Entity)
                .all(db)
                .await?;
            ...
            let transects = model
                ...
                .all(db)
                .await?;
            let mut transect_objs: Vec<crate::routes::private::transects::models::Transect> =
                vec![];

            for transect in &transects {
                let node_objs = transect
                    .find_related(crate::routes::private::transects::nodes::db::Entity)
                    .all(db)
                    .await?;
                let mut nodes = vec![];
                ...
                for node in node_objs {
                    ...
                    plot.image = None;
                    let transect_node =
                        crate::routes::private::transects::nodes::models::TransectNode {
                            plot: Some(plot),
                            order: node.order,
                ...
                transect_objs.push(crate::routes::private::transects::models::Transect {
                    id: transect.id,
    ...
    async fn get_one(db: &DatabaseConnection, id: Uuid) -> Result<Self, DbErr> {
        ...
        let transects = model
            ...
            .all(db)
            .await?;
        let mut transect_objs: Vec<crate::routes::private::transects::models::Transect> = vec![];
