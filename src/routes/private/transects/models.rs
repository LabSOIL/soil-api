use crate::routes::private::plots::models::Plot;
use crate::routes::private::transects::db;
use crate::routes::private::transects::nodes::models::TransectNode;
...
impl CRUDResource for Transect {
    ...
    async fn create(
        ...
        for (i, node) in create_model.nodes.into_iter().enumerate() {
            ...
            let mut plot = crate::routes::private::plots::db::Entity::find()
                .filter(crate::routes::private::plots::db::Column::Id.eq(node.plot_id))
                .one(db)
                .await?
            ...
            let transect_node_active = crate::routes::private::transects::nodes::db::ActiveModel {
                id: Set(Uuid::new_v4()),
                plot_id: Set(plot.id),
                transect_id: Set(transect_id),
                order: Set(i32::try_from(i).unwrap()),
            };
            crate::routes::private::transects::nodes::db::Entity::insert(transect_node_active)
                .exec(db)
                .await?;
        }
    ...
    async fn get_all(
        ...
        let results: Vec<(
            db::Model,
            Vec<crate::routes::private::transects::nodes::db::Model>,
        )> = Self::EntityType::find()
            .filter(condition)
            .order_by(order_column, order_direction)
            .offset(offset)
            .limit(limit)
            .find_with_related(crate::routes::private::transects::nodes::db::Entity)
            .all(db)
            .await?;
        ...
        for (transect_model, nodes) in results {
            ...
            for node in nodes {
                ...
                let mut plot: Plot = crate::routes::private::plots::db::Entity::find()
                    .filter(crate::routes::private::plots::db::Column::Id.eq(node.plot_id))
                    .one(db)
                    .await?
                    .ok_or(DbErr::RecordNotFound("Plot not found".into()))?
            ...
            let area: crate::routes::private::areas::models::Area =
                crate::routes::private::areas::db::Entity::find()
                    .filter(
                        crate::routes::private::areas::db::Column::Id.eq(transect_model.area_id),
                    )
                    .one(db)
                    .await?
    ...
    async fn update(
        ...
        crate::routes::private::transects::nodes::db::Entity::delete_many()
            .filter(crate::routes::private::transects::nodes::db::Column::TransectId.eq(updated.id))
            .exec(db)
            .await?;
